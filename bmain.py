...
from model.bfc import FC
from model.supconloss import SupConLoss
from data.bdataset import MultiDataset
...
if __name__=='__main__':
    ...
    parser.add_argument('-b', '--batch-size', default=2048, type=int, metavar='int')
    ...
    for epoch in range(args.epochs):
        for d_index, d_vecs, _, p_embeddings, p_gos, y, classes in tqdm(trainLoader, leave=False):
            optimizer.zero_grad()
            y_bar, feature, decoded = model(d_index, d_vecs, p_embeddings, p_gos, y, train)

            train_mse = mseLoss(y, y_bar)
            trainLoss = train_mse + 0.1 * supConLoss(feature, classes) + 0.1 * mseLoss(p_gos, decoded)
            trainLoss.backward()
        ...
        with torch.no_grad():
            ...
            for d_index, d_vecs, _, p_embeddings, p_gos, y in testLoader:
                y_bar, _, _ = model(d_index, d_vecs, p_embeddings, p_gos, y, test)
